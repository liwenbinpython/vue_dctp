  ...
  <div class="popupbox">
    ...
    <button type="button" aria-label="Close" class="el-dialog__headerbtn" @click.stop="cancelModal"><i class="el-dialog__close el-icon el-icon-close"></i></button>
      <el-form>
        ...
        <h2 style="margin-left:30px;color:#67C23A" v-else-if="text.status === 'new_stop' && text.msg === 'success'">停止成功</h2>
        <h2 style="margin-left:30px;color:#67C23A" v-else-if="text.status === 'new_start' && text.msg === 'success'">启动成功</h2>
        
        <p style="padding:0 30px;" v-if="text.status === 'stop' && text.msg === 'success'">ID为：{{ text.robot_id }} 的机器人停止成功！</p>
        <p style="padding:0 30px;" v-else-if="text.status === 'start' && text.msg === 'success'">ID为：{{ text.robot_id }} 的机器人启动成功！</p>
        <p style="padding:0 30px;" v-else-if="text.status === 'new_start' && text.msg === 'success'">{{text.content}}</p>
        <p style="padding:0 30px;" v-else-if="text.status === 'new_stop' && text.msg === 'success'">{{text.content}}</p>
        <el-form-item>
