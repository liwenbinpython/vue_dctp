<template>
  <div class="popupbox">
    ...
    <button type="button" aria-label="Close" class="el-dialog__headerbtn" @click.stop="cancelModal"><i class="el-dialog__close el-icon el-icon-close"></i></button>
      <el-form style="margin-top:-20px; width:400px;margin:0 auto;">
        ...
        <el-form-item v-for="(name, value, index) of params.key" :key="index+'+'">
    ...
    private putCurrentParams(){
      ...
      modifyRobotParams(subParams).then((res) => {
        if(res.status === 200){
          this.cancelModal()
          this.getExchange()
          getRobotIndexInfo('strategy',this.params.curPage).then((res) => {
            if(res.status === 200){
