    ...
    <div class="body">
      ...
      <div class="body-content">
        <div class="form">
          ...
          <div class="item">
            <span>购买折扣 : </span>
            <span v-if='data.mall_discount !== 1'>{{ data.mall_discount * 10 }}折</span>
          ...
          <div class="item">
            <span>预估金额 :</span>
            <span class="price">
              <span>{{total.toFixed(4) * data.mall_discount }} {{symbol}}</span>（此价格为预估金额，实际金额已提交订单之后的汇率为准！）
            </span>
          </div>
          <div class="item">
...
import { fetchList, fetchRate, create, del } from "@/api/mall";
...
export default class MallForm extends Vue {
  ...
  private submit() {
    create({
      ...
      this.$router.push({ path: '/mall/detail', query: {mall_order_id: response.data.mall_order_id } });
    });
  }
