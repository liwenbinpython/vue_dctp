    ...
    <div class="home-func">
      <div class="strategy">
        <div class="strategy-row" v-for=" item in list" :key="item.mall_id">
          <div class="list">
            <div class="list-title" style="font-weight:bold;color:#1F2E4E">机器人类型</div>
            <div class="list-content-one">{{item.title}}</div>
          </div>
          <div class="list" style="overflow: hidden;">
            <div class="list-title">可选用策略</div>
            <div class="list-content-three" style="margin-top:30px;overflow-y:auto">{{item.description}}</div>
          </div>
          <div class="list">
          ...
          <div class="list">
              <div class="price">
                <el-button type="shape" style="width:80%" @click="buy(item.mall_id)">立即购买</el-button>
              </div>
          </div>
        </div>
      </div>
...
export default class Robot extends Vue {
  ...
  private fetchData(): void {
    fetchList({ request_type: 1 }).then((response: any) => {
