        ...
        <div class="item_content">
          <el-row>
            ...
            <el-col :span="4">
              ...
              <div
                v-else-if="item.billing_method == 'month'"
                class="grid-content"
              >{{item.duration}}月</div>
              <div v-else class="grid-content">---</div>
            </el-col>
            <el-col :span="6">
              <div class="grid-content" style="border:none;display:flex;align-items:center;justify-content:center;">
                <el-button
                  v-if="item.order_status === 1"
                  @click="renew(item.order_id,item.mall_id)"
                ...
                <el-button
                  ...
                  @click="payment(item.order_id,item.title)"
                  style="width:80px;height:30px;"
                  size="mini"
                  type="danger"
                >立即付款</el-button>
...
export default class userInfo extends Vue {
  ...
  //续费
  private renew(order_id:any,id:any) {
    this.$router.push({ path: '/mall/renew', query: { order_id,id,request_type:'3' }});
  }
  ...
  private del(id: number) {
      MessageBox.confirm(
          ...
          del({mall_order_id: id,}).then((response: any) => {
            if (response.status === 200) {
